...
def main():
    """Main entry point."""
    logger.info("=" * 60)
    logger.info("GERMAN BBL - JOIN DATA")
    logger.info("=" * 60)

    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')

    # Load American players data
    american_data = load_json('bbl_american_stats_latest.json')
    ...
    # Load teams data
    teams_data = load_json('bbl_schedule_latest.json')
    teams_lookup = {}
    if teams_data:
    ...
    hometown_data = load_json('bbl_hometowns_latest.json')
    hometown_lookup = {}
    if hometown_data:
    ...
    # Load box scores to build game logs (if available)
    boxscores_data = load_json('bbl_boxscores_latest.json')
    ...
    for player in players:
        # Skip managers/coaches
        if player.get('position', '').lower() in ['manager', 'coach', 'head coach']:
            continue
